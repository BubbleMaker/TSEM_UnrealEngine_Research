...
\paragraph{Save} permet de sauvegarder le projet.
\paragraph{Source Control} vérifie l'intégrité du projet.
\paragraph{Content} affiche les contenus.
\paragraph{Marketplace} permet de naviguer dans la marché de UE4 pour télécharger du contenu.
\paragraph{Settings} permet de modifier les paramètres du projet.
\paragraph{Matinee} permet de gérer les animations.
\paragraph{Build} génère votre projet.
\paragraph{Compile} compile votre code C++ pour l'éditeur.
\paragraph{Play} permet de jouer directement sur l'éditeur.
\paragraph{Launch} permet de générer la version finale du produit.
...
Les \emph{Blueprints} sont des plans qui permettent d'ajouter des fonctionnalités à un objet ou une classe sans devoir passer par du code C++.
...
\subsection{Configuration de la camera}
